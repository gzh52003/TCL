...
router.post("/login", async (req, res) => {
    ...
    hash.update(password + "xiaowei");
    password = hash.digest("hex");
    let result = await mongo.find("some", { username, password },{field:{password:false}})
    if (result.length > 0) {
        // 判断是否勾选免登陆
        let authorization;
        if (checked) {
...
router.get("/select", async (req, res) => {
    ...
    const result = await mongo.find('some', {}, { skip, limit,field:{password:false} });
...
router.get("/vague",async (req,res)=>{
    ...
    let result = await mongo.find("some",{"username":reg},{field:{password:false}})
    if (result.length > 0) {
...
router.get('/:id', async (req, res) => {
    ...
    // 过滤密码
    const result = await mongo.find('some', { _id: id },{field:{password:false}});
